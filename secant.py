...
if sum_start*sum_end>0:
    ...
    if sum_start<0 and sum_end>0:
        lineer_x1 =start_x+((end_x-start_x)*(-sum_start)/(sum_end-sum_start))
        ...
        if sum_lineer == 0:
            ...
            if sum_start * sum_lineer < 0:
                ...
                start_x = lineer_x1
            lineer_x2 = start_x+((end_x-start_x)*(-sum_start)/(sum_end-sum_start))
    ...
    elif sum_start>0 and sum_end<0:
        lineer_x1 =end_x+((end_x-start_x)*sum_end/(sum_start-sum_end))
        sum_lineer = function(fonksiyon, lineer_x1, max_derece)
        if sum_lineer == 0:
            print("Kök:", lineer_x1)
        else:
            if sum_start * sum_lineer < 0:
                ...
                start_x = lineer_x1
            lineer_x2 =end_x+((end_x-start_x)*sum_end/(sum_start-sum_end))
            sum_lineer = function(fonksiyon, lineer_x2, max_derece)
            while (abs(lineer_x1 - lineer_x2) > epsilon and sum_lineer != 0):
                lineer_x2 =end_x+((end_x-start_x)*sum_end/(sum_start-sum_end))
                sum_lineer = function(fonksiyon, lineer_x2, max_derece)
