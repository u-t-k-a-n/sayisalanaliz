...
if yes_kök=="EVET" and yes_kesir=="EVET":
    sum1=function(fonksiyonlar["fonksiyon"],start_x-delta,max_derece)+\
    ...
    sum=function(fonksiyonlar["fonksiyon"],start_x+delta,max_derece)+\
             (function(fonksiyonlar["kök"],start_x+delta,kök_max)**kök_derece)+ \
             (function(fonksiyonlar["pay"], start_x+delta, pay_max) / function(fonksiyonlar["payda"],start_x+delta, payda_max))
    sum-=sum1
    sum/=2*delta
    ...
    sum -= sum1
    sum /= 2*delta
print("{} noktasındaki 1. türevin değeri:{}".format(start_x,sum))